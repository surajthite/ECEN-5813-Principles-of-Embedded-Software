...
uint32_t * allocate_words(size_t length)
{
	...
	Log_string("\n \r Memory Allocated and starting memory is :"); //Print the starting address of the memory block
	Log_data(ptr, 0);
	Log_string("\n \r Memory Allocated and ending memory is :"); // Print the ending address of the memory block
	Log_data(last_ptr, 0);
	led_switch(2);
	return ptr;
}
